...
| 推奨 OSS | 区分 | 役割 | 採用理由 |
| --- | --- | --- | --- |
| [1. VSCode / VSCodium](002-01.IDE.md) | IDE | 開発者の主作業環境 | GitHub Copilot 利用時は VSCode、OSS 固定時は VSCodiumを選択 |
...
| [9. Forgejo / Gitea / GitHub](002-09.Gitサーバ.md) | Git 管理 | ソース管理、Issue、軽量な運用基盤 | OSS で自己完結（Forgejo/Gitea）または GitHub との組み合わせで選択 |
...
- app: Open WebUI、aider、Git サーバ（Forgejo / Gitea / GitHub）などの業務系サービス
...
- Git サーバ（Forgejo / Gitea / GitHub）と Open WebUI の認証情報は .env ではなく secret ファイルで注入する
...
1. Linux サーバ上に Traefik、Ollama、Open WebUI、Forgejo / Gitea / GitHub を Compose（または外部サービス）で構築する
...
初期段階では Traefik、Ollama、Open WebUI、aider、Forgejo / Gitea / GitHub の最小構成で十分であり、その後に Qdrant と監視基盤を追加する段階導入が、コストと運用負荷の両面で妥当である。
