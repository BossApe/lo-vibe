...
前: [README](README.md) | [一覧](../README.md) | 次: [FR-002-API仕様書](FR-002-API仕様書.md)
...
flowchart TD
    ...
    A --> A1[GET /projects/name-suggestion-profile]
    B --> C[system_overviews 取得]
    C --> D[機能・構成要素抽出]
    D --> E[POST /projects/suggest-name]
    A1 --> A2{profile enabled?}
    A2 -->|yes| A3[PUT /projects/name-suggestion-profile]
    A3 --> B
    A2 -->|no| B
    E --> F[候補名生成]
    F --> G[projectName選択]
    G --> H[POST /projects/init-directory]
    H --> I[prj/{プロジェクト名} ディレクトリ作成（重複チェック）]
    I --> J[テンプレート構造作成]
    J --> K[.keep 作成]
    K --> L[完了]
```
...
- projectName が既存ディレクトリと重複: `VALIDATION_ERROR` (422)
