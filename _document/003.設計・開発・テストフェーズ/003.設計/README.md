...
| A04 | [レビュー観点チェックリスト](レビュー観点チェックリスト.md) | 003.設計 配下の全機能で共通利用するレビュー観点チェックリスト |
...
| # | ドキュメント | 説明 |
| --- | --- | --- |
| 001 | [処理フロー設計書](001.FR-001.システム概要入力/FR-001.処理フロー設計書.md) | FR-001〜FR-004 全体フロー・シーケンス図（正常系） |
| 002 | [インフラ構成図](001.FR-001.システム概要入力/FR-001.インフラ構成図.md) | TK1-1-1 実装時点のサービス・DB 構成図 |
| 003 | [API 仕様書](001.FR-001.システム概要入力/FR-001.API仕様書.md) | POST / GET エンドポイント詳細仕様 |
| 004 | [画面設計書](001.FR-001.システム概要入力/FR-001.画面設計書.md) | SCR-001 UIコンポーネント・バリデーション仕様 |
| 005 | [DB 設計書](001.FR-001.システム概要入力/FR-001.DB設計書.md) | system_overviews テーブル設計 |
| 006 | [テスト仕様書](001.FR-001.システム概要入力/FR-001.テスト仕様書.md) | 単体・内部結合テストケース一覧・受け入れ条件対応 |
| 007 | [エラーハンドリング設計書](001.FR-001.システム概要入力/FR-001.エラーハンドリング設計書.md) | レイヤー別エラー処理責務・エラーコード一覧 |
| 008 | [フロントエンドコンポーネント設計書](001.FR-001.システム概要入力/FR-001.フロントエンドコンポーネント設計書.md) | SCR-001 状態変数・関数仕様・HTML 構造 |
| 009 | [シーケンス図（エラー詳細）](001.FR-001.システム概要入力/FR-001.シーケンス図（エラー詳細）.md) | 異常系 8 パターンのシーケンス図 |
...
| # | ドキュメント | 説明 |
| --- | --- | --- |
| 001 | [処理フロー設計書](002.FR-002-機能抽出・ディレクトリ作成/FR-002-処理フロー設計書.md) | overview 取得から初期ディレクトリ作成までの処理フロー |
| 002 | [API仕様書](002.FR-002-機能抽出・ディレクトリ作成/FR-002-API仕様書.md) | FR-002 API 3本の入出力・エラー仕様 |
...
| 2026-05-09 | 0.4 | レビュー観点チェックリストを共通設計書へ移設し A04 として再番 | Copilot |
| 2026-05-09 | 0.5 | FR-002/FR-003/FR-004 のインフラ構成図を追加 | Copilot |
