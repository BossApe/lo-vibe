...
- 対象タスク: フェーズタスク、イテレーションタスク、チケットタスク
...
- Sort: `Type` → `Iteration` → `Title`
- Filter: `Type` in (`Phase`, `Iteration`, `Ticket`)

Roadmapビューの設定基準:

- Group by: `Phase`
- Timeline: `Start date` 〜 `Target date`
- Filter: `Type` in (`Phase`, `Iteration`)

難易度軸ロードマップ運用:

- GitHub Projects の標準 Roadmap は日付軸前提のため、難易度軸表示は `tools/generate_issue_roadmap_vegalite/` で補完する
- 週次レビュー前に `generate_issue_roadmap_vegalite.sh` を実行し、最新の難易度ロードマップ Markdown を再生成する
- 難易度は `Estimate` を基準に `XS=1, S=2, M=3, L=5, XL=8` へ変換して表示する
- 可視化結果は、レビュー対象の Issue 粒度（`Type` / `Phase` / `Iteration`）と整合していることを確認する

## 6. レビュー運用

- イテレーション開始時: 目標・完了条件・依存関係をレビュー
- 週次: `Blocked` と `Target date` 超過見込みを確認
- イテレーション終了時: 完了判定と次イテレーションへの引継ぎを記録
