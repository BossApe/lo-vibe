...
本ドキュメントは、Musuhi の設計・開発・テストフェーズにおける Forgejo Projects の運用ルールを定義する。
...
Forgejo Projects のカスタムフィールドは以下を標準とする。

本プロジェクトでは、タスクは Draft issue ではなく Forgejo Issue を使用し、親子関係は `Parent issue` / sub-issue で管理する。
...
| `Story Point` | Text | チケットは必須 | `1` / `2` / `3` / `5` / `8` |
| `開始日時` | Text | チケットは必須 | 形式: `yy-mm-dd hh:mm:ss` |
| `終了日時` | Text | チケットは必須 | 形式: `yy-mm-dd hh:mm:ss` |
| `AI Token数` | Text | チケットは必須 | 例: `125,430` |
...
- Forgejo Projects の標準 Roadmap は日付軸前提のため、難易度軸表示は `tools/generate_issue_roadmap/` で補完する
- 週次レビュー前に `go run . -owner <owner> -repo <repo> -project-number <n>` を実行し、最新の難易度ロードマップ HTML を再生成する
- 難易度は `Story Point` を優先し、未設定時のみ `Estimate` を `XS=1, S=2, M=3, L=5, XL=8` に変換して表示する
...
| 2026-05-02 | 0.2 | Forgejo Issue + sub-issue運用を標準化し、Tableビューの標準表示列（Parent issue / Sub-issues progress）を追加 | Copilot |
...
| 2026-05-05 | 0.5 | `Story Point` / `開始日時` / `終了日時` / `AI Token数` をProjects標準項目に追加し、難易度軸はStory Point優先運用へ更新 | Copilot |
