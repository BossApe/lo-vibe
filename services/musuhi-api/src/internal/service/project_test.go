...
type mockExternalGitClient struct {
	mock.Mock
	// mockExternalGitClient は ExternalGitClient のモック実装。
}
...
func (m *mockExternalGitClient) CreateRepositoryAndInitialPush(ctx context.Context, owner, repoName, visibility, localPath, commitMessage string) (*model.ProjectWithExternalResult, error) {
...
func TestProjectService_CreateRepositoryWithExternal_有効な入力でForgejoリポジトリ作成と初回pushを実行する_正常系(t *testing.T) {
	repo := new(mockProjectOverviewRepository)
	git := new(mockExternalGitClient)
	svc := &projectService{overviewRepo: repo, gitClient: git}

	tmpDir := t.TempDir()
	git.On("CreateRepositoryAndInitialPush", mock.Anything, "BossApe", "demo-project", "private", tmpDir, "initial commit").Return(
		&model.ProjectWithExternalResult{
			RepositoryURL:     "http://localhost:13031/BossApe/demo-project.git",
			ExternalProjectID: "demo-project",
			PushStatus:        "success",
		}, nil,
	)
	...
	assert.Equal(t, "http://localhost:13031/BossApe/demo-project.git", got.RepositoryURL)
	git.AssertExpectations(t)
}

func TestProjectService_CreateRepositoryWithExternal_ownerを空で指定してForgejoリポジトリ作成を実行する_異常系(t *testing.T) {
	repo := new(mockProjectOverviewRepository)
	git := new(mockExternalGitClient)
	svc := &projectService{overviewRepo: repo, gitClient: git}
...
func TestProjectService_CreateRepositoryWithExternal_localPathが相対パスの状態でForgejoリポジトリ作成を実行する_異常系(t *testing.T) {
	repo := new(mockProjectOverviewRepository)
	git := new(mockExternalGitClient)
	svc := &projectService{overviewRepo: repo, gitClient: git}
...
func TestProjectService_CreateRepositoryWithExternal_Forgejo側でリポジトリ名重複エラーが発生する_異常系(t *testing.T) {
	repo := new(mockProjectOverviewRepository)
	git := new(mockExternalGitClient)
	svc := &projectService{overviewRepo: repo, gitClient: git}
