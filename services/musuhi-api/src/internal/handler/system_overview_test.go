...
func TestSystemOverviewHandler_GetByID_UUID形式ではないIDでシステム概要を取得する_異常系(t *testing.T) {
	svc := new(mockSystemOverviewService)
	h := NewSystemOverviewHandler(svc)

	svc.On("GetByID", mock.Anything, "not-a-uuid").Return(nil, fmt.Errorf("%w: invalid id format", service.ErrValidation))

	req := httptest.NewRequest(http.MethodGet, "/api/v1/system-overviews/not-a-uuid", nil)
