...
func TestIntegration_UpdateSystemOverview_有効なUUIDと通常のテキストでシステム概要を更新する_正常系(t *testing.T) {
	svc := new(mockSystemOverviewService)
	id := uuid.New()
	content := "結合テスト用更新後システム概要"
	svc.On("Update", mock.Anything, id.String(), content).Return(
		&model.SystemOverview{ID: id, Content: content, CreatedAt: time.Now()}, nil,
	)
	srv := newTestServer(svc)
	defer srv.Close()

	body := `{"content":"結合テスト用更新後システム概要"}`
	req, _ := http.NewRequest(http.MethodPut, srv.URL+"/api/v1/system-overviews/"+id.String(), bytes.NewBufferString(body))
...
func TestIntegration_UpdateSystemOverview_存在しないUUIDでシステム概要を更新する_異常系(t *testing.T) {
	svc := new(mockSystemOverviewService)
	id := uuid.New()
	content := "結合テスト用テキスト"
	svc.On("Update", mock.Anything, id.String(), content).Return(nil, service.ErrNotFound)
	srv := newTestServer(svc)
	defer srv.Close()

	body := `{"content":"結合テスト用テキスト"}`
	req, _ := http.NewRequest(http.MethodPut, srv.URL+"/api/v1/system-overviews/"+id.String(), bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	res, err := http.DefaultClient.Do(req)
	assert.NoError(t, err)
