...
func TestExternalIntegration_ExtractFeatures_実DBのシステム概要から機能一覧を抽出する_正常系(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"overviewId": overviewID})
	res, err := http.Post(srv.URL+"/api/v1/projects/extract-features", "application/json", strings.NewReader(string(body)))
	require.NoError(t, err)
	defer res.Body.Close()
...
func TestExternalIntegration_SuggestName_実DBのシステム概要からプロジェクト名候補を取得する_正常系(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"overviewId": overviewID})
	res, err := http.Post(srv.URL+"/api/v1/projects/suggest-name", "application/json", strings.NewReader(string(body)))
	require.NoError(t, err)
	defer res.Body.Close()
...
func TestExternalIntegration_ExtractFeatures_存在しない概要IDを指定する_異常系(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"overviewId": "00000000-0000-0000-0000-000000000000"})
	res, err := http.Post(srv.URL+"/api/v1/projects/extract-features", "application/json", strings.NewReader(string(body)))
	require.NoError(t, err)
	defer res.Body.Close()
...
func TestExternalIntegration_InitDirectory_有効なパスに初期ディレクトリを作成する_正常系(t *testing.T) {
	...
	res, err := http.Post(srv.URL+"/api/v1/projects/init-directory", "application/json", strings.NewReader(string(body)))
	require.NoError(t, err)
	defer res.Body.Close()
